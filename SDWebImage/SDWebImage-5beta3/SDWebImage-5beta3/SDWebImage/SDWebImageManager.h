...
// 表示缓存和加载程序操作的组合操作。您可以使用它来取消加载进程。
 ...
 * a UIView.
 *
 SDWebImageManager是UIImageView + WebCache 分类依赖的类。
   它将异步下载器（SDWebImageDownloader）与图像缓存存储（SDImageCache）联系起来。
   您可以直接使用此类从Web图像下载中获益，并在另一个上下文中使用缓存而不是UIView。
