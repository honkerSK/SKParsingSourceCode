...
- (void)sd_internalSetImageWithURL:(nullable NSURL *)url
    ...
    if (url) {
        ...
        //SDWebImageManager下载图片 , 调用SDWebImageManager 中 loadImageWithURL:
        id <SDWebImageOperation> operation = [manager loadImageWithURL:url options:options context:context progress:combinedProgressBlock completed:^(UIImage *image, NSData *data, NSError *error, SDImageCacheType cacheType, BOOL finished, NSURL *imageURL) {
            ...
            // case 1a 有image 但是需手动设置image  (SDWebImageAvoidAutoSetImage)
            // case 1b 没有image, 不延迟加载占位图 (SDWebImageDelayPlaceholder)
            ...
            if (image) {
                ...
                // 有image, 也没有手动设置image(SDWebImageAvoidAutoSetImage)
                targetImage = image;
                targetData = data;
            } else if (options & SDWebImageDelayPlaceholder) {
                // case 2b: we got no image and the SDWebImageDelayPlaceholder flag is set
                // 没有image, 也没有设置 延迟加载占位图 (SDWebImageDelayPlaceholder)
